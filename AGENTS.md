...
- Packet-contained runtime-authority, activation, or entitlement receipts are claims, not roots of trust. The target production design requires a separately configured, signed external trust registry that binds source, subject/account, domain, security codes, entitlement, validity period, and signing key. Until that gate is implemented and verified, keep protected sources disabled or blocked for production use; a caller boolean or self-authored packet receipt is never authorization.
- Keep research decisions and realized outcomes in separate append-only ledger streams. Secrets used for HMAC seals come only from runtime environment bytes.
