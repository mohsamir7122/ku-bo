...
- الشركة الكويتية للمقاصة: صفحة `maqasa.com` الرسمية للجمعيات والتوزيعات والملكية والقواعد، مع فصل Access عن الدليل المفسر.
...
الكتالوج الحالي يحتوي 69 تعريف مصدر تتجمع في 63 مجموعة استقلال؛ لا تستخدم الرقمين بالتبادل. بعد استبعاد أسطح البحث والتخزين توجد 60 نطاقًا مرشحًا، منها 54 نطاقًا معلنًا Enabled-public في الكتالوج، و53 نطاق Start URL مميزًا يمكن للمشغل تنفيذه قبل تطبيق الحجز العادل. الخطة الافتراضية تحاول 50 نطاقًا، لكن المحاولة لا تصبح Finding، والنطاق لا يساوي ناشرًا مستقلًا، وإعادة النشر لا تصبح تأكيدًا جديدًا.

حالة القدرات الحالية هي 67 `DEFINED_ONLY` و2 `END_TO_END_TESTED` على Fixtures مولدة و0 `LIVE_OPERATIONAL`. هذه الأرقام تفصل التسجيل عن القدرة التشغيلية، ولا يجوز وصف الـ53 التنفيذية أوالـ54 المعلنة أوالـ60 المرشحة بأنها مصادر حية.
