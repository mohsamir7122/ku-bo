...
الكتالوج الحالي يحتوي 71 تعريف مصدر تتجمع في 65 مجموعة استقلال؛ لا تستخدم الرقمين بالتبادل. بعد استبعاد أسطح البحث والتخزين توجد 61 نطاقًا مرشحًا، منها 54 نطاقًا معلنًا Enabled-public في الكتالوج، و53 نطاق Start URL عامًا مميزًا يمكن للمشغل تنفيذه قبل تطبيق الحجز العادل. الخطة الافتراضية تحاول 50 نطاقًا، لكن المحاولة لا تصبح Finding، والنطاق لا يساوي ناشرًا مستقلًا، وإعادة النشر لا تصبح تأكيدًا جديدًا.

حالة القدرات الحالية هي 69 `DEFINED_ONLY` و2 `END_TO_END_TESTED` على Fixtures مولدة و0 `LIVE_OPERATIONAL`. هذه الأرقام تفصل التسجيل عن القدرة التشغيلية، ولا يجوز وصف المصادر المسجلة أو القابلة للتخطيط بأنها مصادر حية.
